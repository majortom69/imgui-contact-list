...
void LoadContacts(json &contacts)
{
    ...
    else
    {
        ...
        if (outputFile)
        {
            outputFile << json({{"contacts", json::array()}});
            outputFile.close();
        }
...
{
    return std::search(haystack.begin(), haystack.end(), needle.begin(), needle.end(), [](char ch1, char ch2)
    {
...
bool removeContactByPhoneNumber(json &contacts, const std::string &phoneNumber)
{
    if (contacts.find("contacts") != contacts.end())
    {
        ...
        auto it = std::remove_if(contactsArray.begin(), contactsArray.end(),[&phoneNumber](const auto &contact)
        {
...
{
    std::sscanf(birthDate.c_str(), "%d/%d/%d", &month, &day, &year);
    day--;
    month--;
    year -= 1900;
}
...
bool getContactByPhoneNumber(const json &contacts, const std::string &phoneNumber,std::string &firstName, std::string &lastName, std::string &birthDate)
{
    if (contacts.find("contacts") != contacts.end())
    {
        ...
        auto it = std::find_if(contactsArray.begin(), contactsArray.end(),[&phoneNumber](const auto &contact)
        {
            return contact["phone_number"].get<std::string>() == phoneNumber;
        });
...
int main()
{
    ...
    bool showConfirmationEdited = false;
    ...
    const char *items[] = {"Поиск по Фамилии", "Поиск по Имени", "Поиск по Номеру"};
    int searchBy = 0;
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ImGui::SetNextItemWidth(367);
        ImGui::InputText("##Search", searchLastName, sizeof(searchLastName));
        ImGui::SameLine();
        ImGui::SetNextItemWidth(180);
        // Поле поиска по фамилии'
        if (ImGui::BeginCombo("Select Item", items[searchBy]))
        {
            for (int i = 0; i < IM_ARRAYSIZE(items); i++)
            {
                const bool isSelected = (searchBy == i);
        ...
        if (contacts.find("contacts") != contacts.end())
        {
            for (const auto &contact : contacts["contacts"])
            {
                ...
                const std::string &searchText = (searchBy == 0) ? lastName : (searchBy == 1) ? firstName
                                                                                             : phoneNumber;
                // Фильруем контакты
                if (CaseInsensitiveStringSearch(searchText, searchLastName))
                {
        ...
        for (const auto &[letter, contactsForLetter] : contactsByLetter)
        {
            ...
            for (const auto &contact : contactsForLetter)
            {
                ...
                if (ImGui::CollapsingHeader((firstName + " " + lastName).c_str()))
                {
                    ...
                    // // ...
                    // // Set selectedDay, selectedMonth, and selectedYear based on
                    the parsed values
        ...
        if (showInputFrame)
        {
            ...
            ImGui::Text("Номер телефона");
            ImGui::SetNextItemWidth(353);
            ImGui::InputText("##PhoneNumber", newPhoneNumber, sizeof(newPhoneNumber),
            ...
            if (ImGui::Button("Добавить", ImVec2(353, 35)))
            {
                ...
                else
                {
                    ...
                    std::string birthDate = std::to_string(selectedDay + 1) + "/" +
                                            std::to_string(selectedMonth + 1) + "/" + std::to_string(selectedYear + 1900);
        ...
        if (showEditFrame)
        {
            ...
            /*ImGui::SetNextWindowFocus();*/
            ImGui::Begin("Редактировать контакт 1", &showEditFrame,
                         ImGuiWindowFlags_NoResize);
            ImGui::Text("Номер телефона");
            ImGui::SetNextItemWidth(353);
            ImGui::InputText("##PhoneNumber", newPhoneNumberInput,
                             sizeof(newPhoneNumberInput), ImGuiInputTextFlags_CallbackCharFilter, InputTextCallback);
            ImGui::Spacing();
            if (ImGui::Button("Редактировать", ImVec2(353, 35)))
            {
                ...
                std::string editLastName;
                std::string editBirthDate;
                if (getContactByPhoneNumber(contacts, editContactPhone, editFirstName,
                                            editLastName, editBirthDate))
                {
        ...
        if (showEditFrame2)
        {
            ...
            ImGui::SetNextWindowPos(ImVec2(285, 300), ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
            ImGui::SetNextWindowSize(ImVec2(370, 290));
            ImGui::Begin("Редактировать контакт 2", &showEditFrame2,
                         ImGuiWindowFlags_NoResize);
            ImGui::Text("Имя");
            ImGui::SetNextItemWidth(353);
            ...
            ImGui::Combo("День", &parsedDay, [](void *data, int idx, const char **out_text) {
                *out_text = static_cast<std::vector<std::string>*>(data)->at(idx).c_str();
            return true; }, static_cast<void *>(&days), static_cast<int>(days.size()));
            ImGui::SameLine();
            ImGui::SetNextItemWidth(65);
            ImGui::Combo("Месяц", &parsedMonth, [](void *data, int idx, const char **out_text){
                *out_text = static_cast<std::vector<std::string>*>(data)->at(idx).c_str();
            return true; },static_cast<void *>(&months), static_cast<int>(months.size()));
            ImGui::SameLine();
            ImGui::SetNextItemWidth(80);
            ImGui::Combo("Год", &parsedYear, [](void *data, int idx, const char **out_text){
                *out_text = static_cast<std::vector<std::string>*>(data)->at(idx).c_str();
            return true; },static_cast<void *>(&years), static_cast<int>(years.size()));
            ImGui::Spacing();
            if (ImGui::Button("Сохранить", ImVec2(353, 35)))
            {
                ...
                if (phoneNumberExistEdit(contacts, inputPhoneNumberEdit,
                                         tempNumberForEdit))
                {
                    showcConfirmationFail = true;
                    showEditFrame2 = false;
                }
                else if (strlen(newFirstName) == 0 || strlen(newLastName) == 0 ||
                         strlen(newPhoneNumberInput) == 0)
                {
                    showInputFrame = false;
                    showNoName = true;
                }
                else
                {
                    ...
                    std::string birthDate = std::to_string(parsedDay + 1) + "/" +
                                            std::to_string(parsedMonth + 1) + "/" + std::to_string(parsedYear + 1900);
                    newContact["birth_date"] = birthDate;
                    if (contacts.find("contacts") != contacts.end())
                    {
                        contacts["contacts"].push_back(newContact);
        ...
        if (showConfirmationEdited)
        {
            ImGui::SetNextWindowPos(ImVec2(285, 300), ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
            ImGui::SetNextWindowSize(ImVec2(400, 100));
            ImGui::Begin("Подтверждение", &showConfirmation, ImGuiWindowFlags_NoResize);
            ImGui::Text("Контакт измнен успешно!");
            ImGui::SetCursorPosX((ImGui::GetWindowSize().x - ImGui::CalcTextSize("OK").x) * 0.5f);
            if (ImGui::Button("OK"))
            {
                showConfirmationEdited = false;
            }
            ImGui::End();
        }
        ...
        if (showDeleteFrame)
        {
            ...
            {
                std::string deleteContactPhone(newPhoneNumberInput); // Assign the value
                of newPhoneNumber
